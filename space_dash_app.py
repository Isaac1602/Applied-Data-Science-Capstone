...
def scatter_graph(site_dropdown, payload_slider):
    if site_dropdown == 'ALL':
        filter_df = spacex_df[(spacex_df['Payload Mass (kg)']>=payload_slider[0]) &
                                   (spacex_df['Payload Mass (kg)']<=payload_slider[1])]
        scatter_plot = px.scatter(data_frame=filter_df, x = 'Payload Mass (kg)', y= 'class',
                                  color= 'Booster Version Category')
        return scatter_plot
    else:
        specific_df = spacex_df[spacex_df['Launch Site'] == site_dropdown]
        filter_df = specific_df[(specific_df['Payload Mass (kg)']>=payload_slider[0]) & 
